...
class ModelListFilteringMixin(object):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['%slist_filter_by' % self.get_list_prefix()] = dict(
            [(k.rpartition('__')[0] or k.rpartition('__')[2], (k.rpartition('__')[2], v)) for k, v in
             list(filter_query.items())]) or None
        return context
    ...
    def post(self, request, *args, **kwargs):
        ...
        filter_kwargs = dict(
            [("%s%s" % (filter_by_key, f), None) for f, v in list(self.get_filter_query_from_get().items())])
