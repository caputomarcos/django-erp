...
class RenderPluggetTagTestCase(TestCase):
    def setUp(self):
        ...
        self.p1 = Plugget.objects.create(title="Plugget", source="djangoerp.pluggets.pluggets.dummy",
                                         context='{"text": "Hi!"}', region=self.r)
        self.p2 = Plugget.objects.create(title="Another Plugget", source="Something", context='{"text": "Hi!"}',
                                         region=self.r)
        self.p3 = Plugget.objects.create(title="Invalid Plugget", source="Invalid", context='{"text": "Hi!"}',
                                         region=self.r)
    ...
    def test_with_existing_plugget(self):
        ...
        self.assertEqual(
            cleaned_output,
            '<div id="r_plugget-plugget" class="ui fluid card plugget r_plugget">'
            '<div class="content">'
            '<div class="header">Plugget</div>'
            '</div>'
    ...
    def test_with_registered_plugget_source(self):
        ...
        self.assertEqual(
            cleaned_output,
            '<div id="r_another-plugget-plugget" class="ui fluid card plugget r_another-plugget">'
            '<div class="content">'
            '<div class="header">Another Plugget</div>'
            '</div>'
            '<div class="content">Hi!</div>'
            '</div>'
        )
    ...
    def test_with_invalid_plugget_source(self):
        ...
        self.assertEqual(
            cleaned_output,
            '<div id="r_invalid-plugget-plugget" class="ui fluid card plugget r_invalid-plugget">'
            '<div class="content">'
            '<div class="header">Invalid Plugget</div>'
            '</div>'
            '<div class="content">Hi!</div>'
            '</div>'
        )
...
class RenderRegionTagTestCase(TestCase):
    ...
    def test_with_valid_region(self):
        ...
        self.assertEqual(
            cleaned_output,
            '<div id="r-region" class="region"></div>'
        )
